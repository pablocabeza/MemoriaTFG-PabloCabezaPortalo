...
Paula, gracias por ser mi compañera de viaje y por aparecer en el momento que más lo necesitaba. Hemos afrontado este desafío juntos y sin ti no hubiera sido igual.
...
\begin{itemize}
	...
	\item \textbf {Capítulo 7: Conclusiones.} Se observan los resultados obtenidos y se comparan con lo que se esperaba obtener. Se realizan una serie de deducciones tras el tratamiento y el análisis de los datos. Se aplican los conocimientos adquiridos en la carrera y se plantean nuevas líneas de investigación.
...
En el Diagrama de la \textbf {Figura~\ref{fig:diagrama_gantt}} se visualizan las diferentes tareas realizadas junto con la organización en días de las mismas. 
...
Realizamos el parseo de los diferentes datos del paquete para poder insertarlos correctamente en la base de datos.
...
Es posible crear trazados, histogramas, diagramas de barras y cualquier tipo de gráfica como en la Figura ~\ref{fig:matplotlib} con unas líneas de código.
...
El objetivo de este proyecto es descubrir e investigar cómo evoluciona la distribución de Debian a lo largo de su historia y los factores que han cambiado o se han mantenido en releases posteriores junto con su explicación. Para ello hemos seguido estas fases:
\begin{figure}[h]
	\centering
...
En esta fase se requiere un estudio detallado de los diferentes atributos de cada paquete. Asignamos las entidades y las relaciones existentes entre los mismos para poder construir un diagrama válido para el diseño de las bases de datos posteriores.
...
En este tipo de diagrama podemos observar cómo están diseñadas las tablas de la BBDD y como están relacionadas entre sí.
...
En la \textbf {Figura~\ref{fig:flujo creacion BBDD}} podemos visualizar el flujo de trabajo realizado para el diseño y creación de las BBDD. Primero creamos manualmente las BBDD en MySQL Workbench. Creamos un script donde introducimos las credenciales necesarias, diseñamos las tablas en lenguaje SQL e insertamos dichas tablas.
...
\begin{itemize}
	...
	\item \textbf {{Entidad 1 (uno o muchos) Entidad 2}}: en este caso no tenemos claro que entidad debería acoger la Foreing Key por lo que se crea una tabla intermedia.
...
\begin{enumerate}
	\item \textbf{Lectura del archivo.} Las descargas de los releases realizadas en la sección~\ref{sec:Descarga_de_los_releases_de_Debian} las tratamos como ficheros de texto. Por ello debemos leer linea por línea todo el fichero e ir guardando la información relevante de cada paquete.
...
\begin{enumerate}
		...
		\begin{itemize}
			...
			\item \textbf{Files o Checksums-Sha256: } son 2 casos en los que no tenemos comas y vienen varios datos por entidad. Cuando verifiquemos que una linea del paquete empieza por estos 2 casos debemos cambiar la lógica. Saltamos a las siguientes líneas guardando todos los datos en un array. Posteriormente se recorrerá y se insertarán en las diferentes tablas.
...
\begin{enumerate}
	...
	Este sería un ejemplo de una Query que devuelve el número de mantainers total que hay en cada release o BBDD:
...
Y por ello nos preguntamos,  \textbf{¿cuál es el número de mantenedores en cada versión?} Y si no es fijo, \textbf{¿aumenta o disminuye en versiones posteriores?}
...
Como comentamos en las sección ~\ref{sec:pregunta_1}, con el paso del tiempo hay mantenedores nuevos que entran al proyecto y también existen mantenedores que lo abandonan. Por ello nos preguntamos cuantos de los mantenedores anteriores siguen en los releases posteriores y cuál es el porcentaje de los mismos comparando los releases.
...
\end{table}
	...
	En la tabla anterior podemos ver cuál es el valor absoluto de los maintainers de un release que se mantienen en el resto. Se lee de izquierda a derecha comparando el primero con el segundo, el primero con el tercero, el primero con el cuarto, etc.
...
Hemos visto en la sección~\ref{sec:pregunta_4} cuantos mantenedores permanecen activos y cuáles han sido sus aportaciones a lo largo de la evolución de Debian. 
...
La \textbf{vida media} de un maintainer es la suma del número de releases en los que se encuentran cada maintainer dividido entre el número total de maintainers únicos. 
...
Como se observa en los resultados, hay 13 maintainers expertos y 2 no expertos en un total de 15 maintainers. Los que definimos como no expertos son grupos recientes formados por Debian y esta puede ser la explicación de porque no llevan tantas versiones activas.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% CONCLUSIONES %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
En esta sección vamos a comparar los resultados del estudio original, realizado hace veinte años y presentado en la sección~\ref{subsec:articulo debian}, con los obtenidos en este trabajo fin de grado. Además, se ofrecerán y debatirán la consecución de los objetivos planteados en este TFG.

\section{Conclusiones a las cuestiones planteadas}
\label{sec:conclusiones-cuestiones}
En el capítulo~\ref{chap:resultados} de resultados mostramos una serie de respuestas a las preguntas planteadas. En estos capítulos nos gustaría reflexionar acerca de todo el TFG, desarrollando las conclusiones.
...
En el estudio de las primeras cuatro versiones se observa que ha bajado la aportación de estos y solo quedan (desde la primera versión) un 55\% de los mismos. Se intentó calcular la vida media de los maintainers, es decir, cuando su número baja hasta la mitad y se pronosticó que se daría alrededor de 7.5 años.
...
\begin{itemize}
	\item \textbf {Diseñar el diagrama entidad relación.} Este objetivo fue posible gracias al estudio de los diferentes paquetes y comprender las relaciones entre los atributos de estos. Este objetivo era el más importante ya que de él dependen los objetivos posteriores.
	Se encontraron ciertas dificultades para el diseño del diagrama debido a que está formado por una entidad principal que acoge todas las demás. Hubo que adaptarlo para que en un folio se pudiera representar el mismo junto con sus tablas SQL.
	.
...
\begin{enumerate}
	...
	\item Investigar otras distribuciones de Linux u otros sistemas operativos para comparar cómo funcionan, cuáles son las más eficientes y por qué.
